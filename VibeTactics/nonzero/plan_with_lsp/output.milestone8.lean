...
-- Before dispatching structural rules, run lightweight simp normalization.
...
/-- Run a lightweight simp normalization on the current goal. -/
def trySimpNormalize : TacticM Bool :=
  -- Returns true if the goal CHANGED (not necessarily closed)
  -- We use this as a pre-pass before retrying structural rules.
  tryCloseMainGoal do
    evalTactic (← `(tactic|
      simp only [neg_neg, mul_one, one_mul, pow_zero, pow_one,
...
partial def proveNonzeroCore (depth : Nat) : TacticM Unit := do
  ...
  | some (α, e, _z) =>
    ...
    -- simp normalization pre-pass: normalize then retry
    if ← tryCloseMainGoal (evalTactic (← `(tactic|
        simp only [neg_neg, mul_one, one_mul, pow_one,
                   mul_neg, neg_mul] at *))) then return
    -- After simp might change the goal form; retry from scratch
    let goalType2 ← instantiateMVars (← goal.getType)
    if goalType2 != goalType then
      proveNonzeroCore (depth - 1); return
...
-- neg_neg normalization: -(-a) should simplify to a, then recurse
example (a : ℤ) (ha : a ≠ 0) : -(-a) ≠ 0 := by nonzero

-- mul_one / one_mul normalization
example (a : ℝ) (ha : a ≠ 0) : a * 1 ≠ 0 := by nonzero
example (a : ℝ) (ha : a ≠ 0) : 1 * a ≠ 0 := by nonzero

-- pow_one normalization
example (a : ℤ) (ha : a ≠ 0) : a ^ 1 ≠ 0 := by nonzero

-- Combined: simp normalizes then structural rule applies
