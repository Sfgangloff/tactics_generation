/-
  decide_list_theory — Milestone 4: Membership basics

  Builds on Milestone 3 to add:
  - Proving membership goals: a ∈ [a, b, c], a ∈ (a :: l)
  - Proving non-membership: a ∉ []
  - Using `decide` for concrete lists with DecidableEq
-/
...
/-- Try to prove membership goal using simp with membership lemmas -/
def tryMembershipSimp : TacticM Bool := do
  try
    evalTactic (← `(tactic|
      simp only [List.mem_cons, List.mem_singleton, List.not_mem_nil,
                 List.mem_nil_iff, or_false, false_or, true_or, or_true,
...
def decideListTheoryImpl : TacticM Unit := do
  ...
  -- Rule 2: Membership (Milestone 4) - try before length since membership is more specific
...
-- Previous tests still work (Milestones 2-3)
...
-- New tests for membership (Milestone 4)

-- Test 1: Element at head
example : 1 ∈ [1, 2, 3] := by decide_list_theory

-- Test 2: Element in middle
example : 2 ∈ [1, 2, 3] := by decide_list_theory

-- Test 3: Element at tail
example : 3 ∈ [1, 2, 3] := by decide_list_theory

-- Test 4: Not in empty list
example : 1 ∉ ([] : List Nat) := by decide_list_theory

-- Test 5: Membership in singleton
example : 42 ∈ [42] := by decide_list_theory

-- Test 6: Non-membership (element not in list)
example : 4 ∉ [1, 2, 3] := by decide_list_theory

-- Test 7: Membership with variables (head position)
example (a b c : Nat) : a ∈ [a, b, c] := by decide_list_theory

-- Test 8: Membership with variables (other positions) - needs DecidableEq
example (a b c : Nat) : b ∈ [a, b, c] := by decide_list_theory

-- Test 9: Not mem nil (negation form)
example (a : Nat) : ¬(a ∈ ([] : List Nat)) := by decide_list_theory

-- Test 10: Strings (different type with DecidableEq)
example : "hello" ∈ ["hello", "world"] := by decide_list_theory
