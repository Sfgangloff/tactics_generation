...
/-- Decompose a (right-associated) arrow chain `U₁ → … → Uₙ → V`,
    returning `[U₁, …, Uₙ]` and `V`. Non-arrow returns `([], P)`. -/
private partial def spine (P : Expr) : MetaM (Array Expr × Expr) := do
  let rec go (e : Expr) (acc : Array Expr) : MetaM (Array Expr × Expr) := do
    let e' ← whnf e
    match e' with
    | Expr.forallE _ U body _ =>
      go body (acc.push U)
    | _ => return (acc, e')
  go P #[]
  ...
  | Expr.forallE _ A tail _ =>
    ...
    | Expr.forallE _ B body _ =>
      if (← isDefEq body B) then
        withLocalDecl `a .default A fun a =>
        withLocalDecl `b .default B fun b =>
          some <$> mkLambdaFVars #[a, b] b
      else if (← isDefEq body A) then
        withLocalDecl `a .default A fun a =>
        withLocalDecl `b .default B fun b =>
          some <$> mkLambdaFVars #[a, b] a
      else
        return none
    | _ => return none
  | _ => return none

/-- Canonical inhabitant for the *application* pattern
    `A → (A → B) → B`, namely `λ a f, f a`. -/
private def mkApp2 (P : Expr) : MetaM (Option Expr) := do
  let P' ← whnf P
  match P' with
  | Expr.forallE _ A tail _ =>
    let tail' ← whnf tail
    match tail' with
    | Expr.forallE _ AtoB body _ =>
      let AtoB' ← whnf AtoB
      match AtoB' with
      | Expr.forallE _ A' B _ =>
        if (← isDefEq A' A) then
          if (← isDefEq body B) then
            -- λ a f, f a
            withLocalDecl `a .default A fun a =>
            withLocalDecl `f .default AtoB fun f => do
              let app := mkApp f a
              some <$> mkLambdaFVars #[a, f] app
          else
            return none
        else
          return none
...
/-- General n-ary projection (structurally recursive):
    If `P ≡ U₁ → … → Uₙ → V` and for some `i` we have `V ≡ Uᵢ` definitionally,
    synthesize `λ x₁ … xₙ, xᵢ`. -/
private partial def mkProjN (P : Expr) : MetaM (Option Expr) := do
  let (us, V) ← spine P
  if us.isEmpty then
    return none
  ...
  /- find index i such that Uᵢ ≡ V by structural recursion on rem = remaining elements -/
  let rec findAux (idx : Nat) (rem : Nat) : MetaM (Option Nat) := do
    if rem == 0 then
      return none
    else if h : idx < us.size then
      let Ui := us[idx]
      if (← isDefEq Ui V) then
        return some idx
      else
        findAux (idx + 1) (rem - 1)
    else
  ...
  let some i ← findAux 0 n | return none

  /- build nested local declarations and return λ x₁ … xₙ, xᵢ
     We recurse on rem = remaining binders to guarantee termination. -/
  let rec buildAux (idx : Nat) (rem : Nat) (acc : Array Expr) : MetaM Expr := do
    if rem == 0 then
      -- no more binders; pick body = acc[i]
      if h : i < acc.size then
        let body := acc[i]
        mkLambdaFVars acc body
      else
        mkLambdaFVars acc (mkConst ``False) -- unreachable fallback
    else if h : idx < us.size then
      let U := us[idx]
      withLocalDecl (Name.mkSimple s!"x{idx}") .default U fun x => do
        buildAux (idx + 1) (rem - 1) (acc.push x)
    else
      mkLambdaFVars acc (mkConst ``False)    -- unreachable fallback

  some <$> buildAux 0 n #[]
/-- Build a one-argument arrow `U → V`.
    If `V ≡ U`, return identity; else if some local `e : V` exists,
    return the constant `λ _ : U => e`. -/
...
/-- Predicate: decide (constructively) if `P` is inhabitable given `locals`.
    Mirrors the constructive builder. -/
private partial def inhabitable (P : Expr) (locals : Array Expr) : MetaM Bool := do
  let P' ← whnf P
  -- by local
  for e in locals do
    if (← isDefEq (← inferType e) P') then
      return true
  -- projections / application pattern / one-arg arrow
  if let some _ ← mkProj2 P' then return true
  if let some _ ← mkApp2 P' then return true
  if let some _ ← mkProjN P' then return true
  if let some _ ← mkArrow1 P' locals then return true
  -- elimination pattern: ((U₁ → … → Uₙ → V) → V)
  match P' with
  | Expr.forallE _ K V _ =>
    let (us, cod) ← spine K
    if !us.isEmpty ∧ (← isDefEq cod V) then
      for U in us do
        if !(← inhabitable U locals) then
          return false
      return true
    else
      return false
...
/-- Builder: constructive inhabitants for implicational `Prop`. -/
private partial def mkInhabitant (P : Expr) (locals : Array Expr) : MetaM (Option Expr) := do
  let P' ← whnf P
  -- by local
  for e in locals do
    if (← isDefEq (← inferType e) P') then
      return some e
  -- projections / application pattern / one-arg arrow
  if let some t ← mkProj2 P' then return some t
  if let some t ← mkApp2 P' then return some t
  if let some t ← mkProjN P' then return some t
  if let some t ← mkArrow1 P' locals then return some t
  -- elimination pattern
  match P' with
  | Expr.forallE _ K V _ =>
    let (us, cod) ← spine K
    if !us.isEmpty ∧ (← isDefEq cod V) then
      -- recursively build u₁ … uₙ
      let mut built : Array Expr := #[]
      for U in us do
        if let some ui ← mkInhabitant U locals then
          built := built.push ui
        else
          return none
      let K' ← whnf K
      withLocalDecl `k .default K' fun k => do
        let app ← mkAppM' k built
        if (← isDefEq (← inferType app) V) then
          let lam ← mkLambdaFVars #[k] app
          return some lam
        else
          return none
    else
...
/-- Close one subgoal constructively (guarded). -/
private def closeOne (sg : MVarId) (locals : Array Expr) : MetaM Bool := do
  let need ← sg.getType
  if !(← inhabitable need locals) then return false
  if let some t ← mkInhabitant need locals then
    sg.assign t
    return true
  return false

/-- Constructive tactic driver. -/
def intro_all_then_assumption : TacticM Unit := do
  ...
  withMainContext do
    ...
    -- try recursive solver
    if (← closeOne g locals #[]) then
      setGoals []
      return

    -- attempt to apply all locals
    for e in locals do
      let eType ← inferType e
      if (← isDefEq eType tgt) then
        try
          let mvarIds ← g.apply e
          try
            setGoals mvarIds
            allGoals assumption
            return
    ...
    -- no solution found; leave goal as-is
    pure ()

where
  closeOne (g : MVarId) (locals : Array Expr) (visited: Array MVarId) : MetaM Bool := do
    go g locals visited

  go (g : MVarId) (locals : Array Expr) (visited: Array MVarId) : MetaM Bool := do
    if visited.contains g then return false
    let visited := visited.push g
    locals.anyM (fun e => do
      try
        let mvarIds ← g.apply e
        if mvarIds.isEmpty then
          return true
        else
          mvarIds.allM (fun mvarId =>
            if visited.contains mvarId then
              return false
            else
              go mvarId locals visited)
      catch _ => return false)
partial def closeOneAux (gid : MVarId) (locals : Array Expr) : MetaM Bool := do
  withMVarContext gid do
    let goalTy ← whnf (← gid.getType)

    -- try exact from locals
    for e in locals do
      if (← isDefEq (← inferType e) goalTy) then
        gid.assign e
        return true

    -- if goal is a function/forall, introduce and solve recursively
    match goalTy with
    | Expr.forallE _ _ _ _ =>
        let (xs, g') ← gid.intros
        let locals' := locals ++ xs.map mkFVar
        if (← closeOneAux g' locals') then
          return true
        else
          return false
    | _ => pure ()

    -- try applying each local and close all produced subgoals
    for h in locals do
      let s ← saveState
      try
        let subs ← gid.apply h
        let mut ok := true
        for sg in subs do
          if !(← closeOneAux sg locals) then
            ok := false
            break
        if ok then
          return true
    ...
    return false
partial def solve (gid : MVarId) (ls : Array Expr) : MetaM Bool := do
  withMVarContext gid do
    let tgt ← whnf (← gid.getType)

    -- try exact with locals
    for e in ls do
      if (← isDefEq (← inferType e) tgt) then
        gid.assign e
        return true

    -- if goal is a function, introduce an argument and continue
    if tgt.isForall then
      let st ← saveState
      try
        let (fvarId, g') ← gid.intro1
        let binder := mkFVar fvarId
        if (← solve g' (ls.push binder)) then
          return true
        else
          st.restore
          return false
    ...
    -- otherwise, try applying locals that are functions to reach the target
    for h in ls do
      let st ← saveState
      try
        let subgs ← gid.apply h
...
def intro_all_then_assumption_core : TacticM Unit := do
  let g ← getMainGoal
  let locals ← withMVarContext g do
    pure <| (← getLCtx).getFVarIds.map mkFVar
  if (← solve g locals) then
    setGoals []
    return
  throwError "intro_all_then_assumption: goal is not constructively inhabitable from current context"

elab "intro_all_then_assumption" : tactic => intro_all_then_assumption_core
