/-
  decide_list_theory — Milestone 6: Structural induction

  Builds on Milestone 5 to add:
  - Handle universally quantified goals over list variables
  - Structural induction for: ∀ l, l ++ [] = l
  - Omega shortcut for:       ∀ l, length l ≥ 0
-/
...
/-- Try to prove ∀ l, P l using intro + omega.
    Handles trivially arithmetic goals like length l ≥ 0. -/
...
/-- Try to prove ∀ l, P l by intro + simp.
    Handles: ∀ l, l ++ [] = l, ∀ l, [] ++ l = l (no induction needed). -/
def tryForallSimp : TacticM Bool := do
  -- Try simp with common list lemmas (without `only` to use full simp set)
  try
...
/-- Try to prove ∀ l, P l by intro + structural induction + simp_all.
    Handles goals requiring induction over list structure.
    Uses `<;>` combinator to avoid `with` parse issues inside tactic quotes. -/
...
/-- Try to prove ∀ l, P l by intro + induction + simp + omega.
    Handles hybrid numeric/list goals per case. -/
...
/-- Main tactic implementation (Milestone 6) -/
def decideListTheoryImpl : TacticM Unit := do
  ...
  -- Rule 0: Universally quantified list goals (Milestone 6) — processed first
  -- so that inner classifiers (involvesLength etc.) don't fire on ∀ l, length l ≥ 0
...
-- Previous tests still work (Milestones 2-5)
...
-- New tests for structural induction (Milestone 6)

-- Test 1: append-nil identity (classic induction)
example : ∀ (l : List Nat), l ++ [] = l := by decide_list_theory

-- Test 2: nil-append identity (simpler, no induction needed)
example : ∀ (l : List Nat), [] ++ l = l := by decide_list_theory

-- Test 3: length ≥ 0 (trivially true by omega on Nat)
example : ∀ (l : List Nat), l.length ≥ 0 := by decide_list_theory

-- Test 4: 0 ≤ length (alternate notation)
